...
cnt_data <-
  data |> 
  count(re_buy, pet_type) |> 
  mutate(pet_type = reorder_within(pet_type, n, re_buy))

cnt_data |> 
  ggplot(aes(x = pet_type, y = n, fill = re_buy)) +
  geom_col(show.legend = F) + 
  facet_wrap(vars(re_buy), scales = "free_x") +
  scale_x_reordered() +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 199)) +
  theme_classic() +
  theme(
  ...
  labs(
    title = "Most rebought items are for cats and dogs",
    subtitle = "They are also most sold without a rebuy",
    x = "Pet type",
...
As the majority of items is being robought, the marketing team should implement
